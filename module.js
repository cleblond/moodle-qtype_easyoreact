...
M.qtype_easyoreact={
    insert_easyoreact_applet : function(Y, toreplaceid, appletid, appletname, topnode,
...
//        var javaparams = new Array();
        ...
        if (readonly) {
//            easyoreactoptions[easyoreactoptions.length] = "false";  ///crl changed depict to true
//	     javaparams.menubar = "false";	    
//            easyoreactoptions[easyoreactoptions.length + 1] = "false";  ///crl changed depict to true
//	    easyoreactoptions[easyoreactoptions.length] = Y.one(topnode+' input.mol').get('value');  ///crl changed depict to true
	    easyoreactoptions[easyoreactoptions.length] = Y.one(topnode+' input.mol').get('value');  ///crl 
...
//            javaparams[javaparams.length+1] = "menubar";   ///crl changes options to viewonly 
        ...
        if (!this.show_java(toreplaceid, appletid, appletname, appleturl,
                                                            660, 460, 'chemaxon.marvin.applet.JMSketchLaunch', javaparams, stripped_answer_id)) {
...
//		this.find_java_applet(appletname).setMol(s, 'smiles');
//		alert(appletname + appletid + toreplaceid);

//		document[appletname].setMol(s, 'smiles');
//		var $junk = document.getElementById(appletid).setMol(s, 'smiles');
//		setTimeout(function() {document.getElementById(appletid).setMol(s, 'smiles');},200)
		var inputdiv = Y.one(topnode);
            	inputdiv.ancestor('form').on('submit', function (){
//		var s = this.find_java_applet(appletname).getMol("mol");
/*
//    var s = document.MSketch.getMol(format);
                Y.one(topnode+' input.answer').set('value', this.find_java_applet(appletname).smiles()); */
                Y.one(topnode+' input.answer').set('value', this.find_java_applet(appletname).getMol("cxsmiles:u"));

//		 Y.one(topnode+' input.answer').set('value', this.find_java_applet(appletname).getMol("smiles"));
                 
//		var s = this.find_java_applet(appletname).getMol("mol");
//		s = local2unix(s);

//		var strvalue = "" + this.find_java_applet(appletname).getMol("mrv");
...
//                Y.one(topnode+' input.easyoreact').set('value', s);
...
*/
    ...
    show_error : function (Y, topnode) {
        var errormessage = '<span class ="javawarning">'
            +M.util.get_string('enablejava', 'qtype_easyoreact')+
            '</span>';
...
//	newApplet.setAttribute('codebase','../../../easyoreact');
